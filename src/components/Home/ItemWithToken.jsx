...
// import ItemId from "../../stateManagement/itemId";
...
const Item = () => {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    `https://oldphonedeals.azurewebsites.net/home/item/`,
    (url) => fetcher(url, "post", { phoneId: phoneid }),
    { revalidateOnMount: true }
  );

  const handleButtonClick = () => {
    mutate(`https://oldphonedeals.azurewebsites.net/home/item/`, { phoneId: phoneid }, "post");
  };
  ...
  useEffect(() => {
    ...
    if (cartData && JSON.parse(cartData).length > 0) {
      ...
      for (let i = 0; i < cart.length; i++) {
        const obj = cart[i];
        if (data?.data?._id === obj.id) {
          setQuantity(obj.order);
        }
      }
    } else {
      setQuantity(0);
